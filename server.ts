...
async function startServer() {
      ...
      try {
        ...
        if (!testRes.ok) {
          ...
          if (testRes.status === 401 || testRes.status === 403) {
            logError(`OPENAI_APIKEY_REJECTED: ${text}`);
            return res.status(400).json({ error: "That key was rejected by OpenAI. Check it and try again." });
          }
        }
      } catch (e: any) {
        logError(`OPENAI_APIKEY_VALIDATION_SOFT_FAIL: ${e.message}`);
      }
      setOpenAiApiKey(key);
      logCommand("OPENAI_APIKEY_SAVED");
      res.json({ ok: true, hasOpenAiApiKey: true });
    } catch (e: any) {
      logError(`OPENAI_APIKEY_SAVE_ERROR: ${e?.message || e}`);
      res.status(500).json({ error: e?.message || "Failed to save OpenAI API key." });
    }
  });

  app.post("/api/config/anthropic", async (req, res) => {
    try {
      const key: string = (req.body?.apiKey ?? "").toString().trim();
      if (!key) {
        return res.status(400).json({ error: "API key is required." });
      ...
      try {
        ...
        if (!testRes.ok) {
          const text = await testRes.text();
          if (testRes.status === 401 || testRes.status === 403) {
            logError(`ANTHROPIC_APIKEY_REJECTED: ${text}`);
            return res.status(400).json({ error: "That key was rejected by Anthropic. Check it and try again." });
          }
        }
      } catch (e: any) {
        logError(`ANTHROPIC_APIKEY_VALIDATION_SOFT_FAIL: ${e.message}`);
      }
      setAnthropicApiKey(key);
      logCommand("ANTHROPIC_APIKEY_SAVED");
      res.json({ ok: true, hasAnthropicApiKey: true });
    } catch (e: any) {
      logError(`ANTHROPIC_APIKEY_SAVE_ERROR: ${e?.message || e}`);
      res.status(500).json({ error: e?.message || "Failed to save Anthropic API key." });
    }
  });
  ...
  function convertGeminiSchemaToOpenAi(schema: any): any {
    ...
    if (result.properties) {
      ...
      for (const key of Object.keys(result.properties)) {
        newProps[key] = convertGeminiSchemaToOpenAi(result.properties[key]);
      }
      result.properties = newProps;
    }
    if (result.items) {
      result.items = convertGeminiSchemaToOpenAi(result.items);
    }
    return result;
  }

  function resample16To24(buffer16: Buffer): Buffer {
    const numSamples16 = buffer16.length / 2;
    const numSamples24 = Math.round(numSamples16 * 1.5);
    const buffer24 = Buffer.alloc(numSamples24 * 2);
    for (let i = 0; i < numSamples24; i++) {
      const index16 = i / 1.5;
      const leftIndex = Math.floor(index16);
      const rightIndex = Math.min(leftIndex + 1, numSamples16 - 1);
      const fraction = index16 - leftIndex;
      if (leftIndex * 2 + 1 >= buffer16.length || rightIndex * 2 + 1 >= buffer16.length) {
        break;
      }
      const leftVal = buffer16.readInt16LE(leftIndex * 2);
      const rightVal = buffer16.readInt16LE(rightIndex * 2);
      const interpolated = Math.round(leftVal + fraction * (rightVal - leftVal));
      buffer24.writeInt16LE(interpolated, i * 2);
    }
    return buffer24;
  }
  ...
  function addWavHeader(pcmBuffer: Buffer, sampleRate: number): Buffer {
    ...
    header.write("RIFF", 0);
    header.writeInt32LE(byteLength + 36, 4);
    header.write("WAVE", 8);
    header.write("fmt ", 12);
    ...
    header.writeInt32LE(byteLength, 40);
  ...
  wss.on("connection", async (clientWs) => {
    ...
    try {
      ...
      const geminiTools = [
        ...
        {
          name: "searchYouTube",
          description: "Search YouTube content in browser.",
          parameters: { type: Type.OBJECT, properties: { query: { type: Type.STRING, description: "Search query." } }, required: ["query"] }
        },
        {
          name: "searchGoogle",
          description: "Search Google details in browser.",
          parameters: { type: Type.OBJECT, properties: { query: { type: Type.STRING, description: "Search query." } }, required: ["query"] }
        },
        {
          name: "searchGitHub",
          description: "Search GitHub code/repositories in browser.",
          parameters: { type: Type.OBJECT, properties: { query: { type: Type.STRING, description: "Search query." } }, required: ["query"] }
        },
        ...
        {
          name: "createFile",
          description: "Create a new text file at path with custom content.",
          parameters: { type: Type.OBJECT, properties: { path: { type: Type.STRING, description: "File path." }, content: { type: Type.STRING, description: "File content text." } }, required: ["path"] }
        },
        {
          name: "readFile",
          description: "Read text contents from a file path.",
          parameters: { type: Type.OBJECT, properties: { path: { type: Type.STRING, description: "Target path." } }, required: ["path"] }
        },
        {
          name: "renameFile",
          description: "Rename or move a file.",
          parameters: { type: Type.OBJECT, properties: { oldPath: { type: Type.STRING }, newPath: { type: Type.STRING } }, required: ["oldPath", "newPath"] }
        },
        {
          name: "deleteFile",
          description: "Move a file to Recycle Bin or delete permanently.",
          parameters: { type: Type.OBJECT, properties: { path: { type: Type.STRING }, permanent: { type: Type.BOOLEAN, description: "If true, bypass recycle bin." } }, required: ["path"] }
        },
        {
          name: "moveFile",
          description: "Move a file from source to dest.",
          parameters: { type: Type.OBJECT, properties: { src: { type: Type.STRING }, dest: { type: Type.STRING } }, required: ["src", "dest"] }
        },
        {
          name: "openFolder",
        ...
        {
          name: "listFiles",
          description: "List directory files inside directory.",
          parameters: { type: Type.OBJECT, properties: { path: { type: Type.STRING } }, required: ["path"] }
        },
        {
          name: "searchFiles",
          description: "Search for files inside a directory by name substring or extension.",
          parameters: { type: Type.OBJECT, properties: { directory: { type: Type.STRING }, query: { type: Type.STRING }, extension: { type: Type.STRING } }, required: ["directory"] }
        },

        // ======== LOCAL VOLUME & POWER OPERATIONS ========
        {
          name: "volumeUp",
          description: "Increase master system volume by set percent step (default 5).",
          parameters: { type: Type.OBJECT, properties: { step: { type: Type.NUMBER } } }
        },
        {
          name: "volumeDown",
          description: "Decrease master system volume by set percent step (default 5).",
          parameters: { type: Type.OBJECT, properties: { step: { type: Type.NUMBER } } }
        },
        {
          name: "muteToggle",
        ...
        {
          name: "setVolume",
          description: "Set master volume to exact percent level (0-100).",
          parameters: { type: Type.OBJECT, properties: { level: { type: Type.NUMBER } }, required: ["level"] }
        },
        {
          name: "requestPowerAction",
          description: "Acquire confirmation token for a protected power action (shutdown, restart, sleep, lock). MUST be done before executePowerAction.",
          parameters: { type: Type.OBJECT, properties: { action: { type: Type.STRING, enum: ["shutdown", "restart", "sleep", "lock"] } }, required: ["action"] }
        },
        {
          name: "executePowerAction",
          description: "Execute a protected power action with a valid verification token.",
        ...
        {
          name: "minimizeWindow",
          description: "Minimize the current active window or named application window.",
          parameters: { type: Type.OBJECT, properties: { title: { type: Type.STRING } } }
        },
        {
          name: "maximizeWindow",
          description: "Maximize the current active window or named application window.",
          parameters: { type: Type.OBJECT, properties: { title: { type: Type.STRING } } }
        },
        {
          name: "closeWindow",
          description: "Close the current active window or named application window.",
          parameters: { type: Type.OBJECT, properties: { title: { type: Type.STRING } } }
        },
        {
          name: "switchApplication",
          description: "Bring a named application window to focus/foreground.",
        ...
        {
          name: "copySelected",
          description: "Trigger Ctrl+C and read the resulting text content from system clipboard.",
          parameters: { type: Type.OBJECT, properties: {} }
        },
        {
          name: "pasteClipboard",
          description: "Paste set text content into active window input (writes clipboard then triggers Ctrl+V).",
        ...
        // ======== LOCAL SCREENSHOT & OCR OPERATIONS ========
        {
          name: "takeScreenshot",
          description: "Take a temporary screenshot and return its dimensions.",
          parameters: { type: Type.OBJECT, properties: {} }
        },
        {
          name: "saveScreenshot",
          description: "Save system screenshot to target file path.",
          parameters: { type: Type.OBJECT, properties: { path: { type: Type.STRING } }, required: ["path"] }
        },
        {
          name: "analyzeScreenshot",
          description: "Perform OCR and details analysis on active screen.",
          parameters: { type: Type.OBJECT, properties: {} }
        },
        {
          name: "readScreen",
          description: "OCR the active window text content and return its title.",
          parameters: { type: Type.OBJECT, properties: {} }
        },

        // ======== DESKTOP PLAYWRIGHT BROWSER AUTOMATION ========
        {
          name: "desktopBrowserOpen",
          description: "Launch/Open the automated desktop Playwright browser at target URL.",
          parameters: { type: Type.OBJECT, properties: { url: { type: Type.STRING } } }
        },
        {
          name: "desktopBrowserNavigate",
          description: "Navigate active automated browser tab to target URL.",
          parameters: { type: Type.OBJECT, properties: { url: { type: Type.STRING } }, required: ["url"] }
        },
        {
          name: "desktopBrowserOpenTab",
          description: "Open a new tab in the automated browser.",
          parameters: { type: Type.OBJECT, properties: { url: { type: Type.STRING } } }
        },
        {
          name: "desktopBrowserCloseTab",
          description: "Close active or specified index tab in the automated browser.",
          parameters: { type: Type.OBJECT, properties: { index: { type: Type.NUMBER } } }
        },
        {
          name: "desktopBrowserSearch",
          description: "Type search query and execute search on search engine page.",
          parameters: { type: Type.OBJECT, properties: { query: { type: Type.STRING } }, required: ["query"] }
        },
        {
          name: "desktopBrowserClick",
          description: "Click a target button/link by selector.",
          parameters: { type: Type.OBJECT, properties: { selector: { type: Type.STRING } }, required: ["selector"] }
        },
        {
          name: "desktopBrowserType",
          description: "Type text into a target input box field by selector.",
          parameters: { type: Type.OBJECT, properties: { selector: { type: Type.STRING }, text: { type: Type.STRING } }, required: ["selector", "text"] }
        },
        {
          name: "desktopBrowserFillForm",
          description: "Fill multiple input fields by selectors inside a page form.",
          parameters: { type: Type.OBJECT, properties: { fields: { type: Type.OBJECT, description: "Map of selector:value" } }, required: ["fields"] }
        },
        {
        ...
        },
        {
          ...
          description: "Scroll active automated browser view vertically up or down.",
          parameters: { type: Type.OBJECT, properties: { direction: { type: Type.STRING, enum: ["up", "down"] }, amount: { type: Type.NUMBER } } }
        },

        // ======== LOCAL CODING ASSISTANCE ========
        {
          name: "createPythonFile",
          description: "Create a Python file with source code content.",
          parameters: { type: Type.OBJECT, properties: { path: { type: Type.STRING }, code: { type: Type.STRING } }, required: ["path", "code"] }
        },
        {
          name: "runPythonScript",
          description: "Execute a local Python script and return stdout/stderr.",
          parameters: { type: Type.OBJECT, properties: { path: { type: Type.STRING }, args: { type: Type.ARRAY, items: { type: Type.STRING } } }, required: ["path"] }
        },
        {
          name: "createProjectFolder",
          description: "Create a new project folder layout with multiple subfolders.",
          parameters: { type: Type.OBJECT, properties: { path: { type: Type.STRING }, folders: { type: Type.ARRAY, items: { type: Type.STRING } } }, required: ["path", "folders"] }
        },
        {
          name: "writeCodeFile",
          description: "Write code source contents into any file path (any language).",
          parameters: { type: Type.OBJECT, properties: { path: { type: Type.STRING }, code: { type: Type.STRING } }, required: ["path", "code"] }
        },
        ...
        {
          name: "systemInfo",
          description: "Get basic Windows PC hardware specs (CPU, RAM, main disks, OS build, system uptime).",
          parameters: { type: Type.OBJECT, properties: {} }
        },
        {
          name: "gpuInfo",
          description: "Get GPU hardware details and NVIDIA VRAM usage statistics.",
          parameters: { type: Type.OBJECT, properties: {} }
        },
        {
          name: "temperatureInfo",
          description: "Get CPU/GPU sensor temperature details.",
          parameters: { type: Type.OBJECT, properties: {} }
        },

        // ======== LOCAL DISPLAY CONTROL (V2) ========
        {
          name: "brightnessUp",
      ...
      if (provider === "openai") {
        ...
        if (!openaiApiKey) {
          clientWs.send(JSON.stringify({
            type: "error",
            error: "NO_API_KEY: Add your OpenAI API key in Settings to start talking to MYRAA."
          }));
          clientWs.close();
          return;
        }

        clientWs.send(JSON.stringify({ type: "status", status: "connecting_gemini" })); // Keep connecting_gemini status to show activity

        const openaiTools = geminiTools.map(t => ({
          type: "function",
          name: t.name,
          description: t.description,
          parameters: convertGeminiSchemaToOpenAi(t.parameters)
        }));
        ...
        openaiWs.on("message", (data) => {
          try {
            ...
            if (event.type === "response.audio_transcript.delta" && event.delta) {
              clientWs.send(JSON.stringify({ type: "transcription", role: "model", text: event.delta }));
              currentModelResponseText += event.delta;
            }
            ...
            if (event.type === "response.done") {
              ...
              if (dialogueHistory.length >= 2) {
                (async () => {
                  try {
                    const updated = await processConversationSlice(getGeminiApiKey() || openaiApiKey, dialogueHistory);
                    if (updated) {
                      clientWs.send(JSON.stringify({ type: "memory_sync", memories: updated }));
                    }
                  } catch (err) {
                    console.error("[Memory Sync] OpenAI background error:", err);
                  }
                })();
              }
            }
                    ...
                    if (category && text) {
                      ...
                      openaiWs.send(JSON.stringify({
                        ...
                        item: {
                          ...
                          call_id: call_id,
                          output: JSON.stringify({ result: "Memory successfully captured and persisted in connections core." })
                        }
                      }));
            ...
            if (event.type === "error") {
              ...
              clientWs.send(JSON.stringify({ type: "error", error: event.error?.message || "OpenAI error" }));
            }
          } catch (e) {
            console.error("Failed to parse OpenAI Realtime message:", e);
          }
        });
        ...
        openaiWs.on("error", (err) => {
          ...
          clientWs.send(JSON.stringify({ type: "error", error: "OpenAI Realtime WS transport error" }));
        });
        ...
        clientWs.on("message", (rawMsg) => {
          try {
            ...
            if (msg.audio) {
              const buffer16 = Buffer.from(msg.audio, "base64");
              const buffer24 = resample16To24(buffer16);
              openaiWs.send(JSON.stringify({
                type: "input_audio_buffer.append",
                audio: buffer24.toString("base64")
              }));
              ...
              openaiWs.send(JSON.stringify({
                ...
                item: {
                  type: "function_call_output",
                  call_id: msg.id,
                  output: JSON.stringify(msg.output)
                }
              }));
              openaiWs.send(JSON.stringify({ type: "response.create" }));
            }
        ...
        clientWs.on("close", () => {
          console.log("Client disconnected, closing OpenAI session");
          try {
            openaiWs.close();
          } catch (e) {}
        });
      ...
      if (provider === "anthropic") {
        ...
        if (!anthropicApiKey) {
          clientWs.send(JSON.stringify({
            type: "error",
            error: "NO_API_KEY: Add your Anthropic API key in Settings to start talking to MYRAA."
          }));
          clientWs.close();
          return;
        }
        ...
        const handleAnthropicText = async (userText: string) => {
          ...
          try {
            ...
            const response = await fetch("https://api.anthropic.com/v1/messages", {
              ...
              body: JSON.stringify({
                ...
                messages: dialogueHistory.map(h => ({
            ...
            dialogueHistory.push({ role: "model", text: modelText });
            ...
            if (openAiKey) {
              try {
                ...
                if (ttsRes.ok) {
                  ...
                  const chunkSize = 4800; // ~100ms chunks at 24kHz PCM16 (24000 * 2 bytes)
                  for (let offset = 0; offset < buffer.length; offset += chunkSize) {
                    const chunk = buffer.slice(offset, offset + chunkSize);
                    clientWs.send(JSON.stringify({ type: "audio", audio: chunk.toString("base64") }));
            ...
            if (dialogueHistory.length >= 2) {
              (async () => {
                try {
                  const updated = await processConversationSlice(getGeminiApiKey() || openAiKey || anthropicApiKey, dialogueHistory);
                  if (updated) {
                    clientWs.send(JSON.stringify({ type: "memory_sync", memories: updated }));
                  }
                } catch (err) {
                  console.error("[Memory Sync] Anthropic background error:", err);
                }
              })();
            }

          } catch (e: any) {
            console.error("Anthropic flow error:", e);
          ...
          try {
            ...
            if (msg.audio) {
              ...
              silenceTimer = setTimeout(async () => {
                ...
                if (openAiKey) {
                  try {
                    const wavBuffer = addWavHeader(totalBuffer, 16000);
                    ...
                    if (whisperRes.ok) {
                      ...
                      if (transcript) {
                        handleAnthropicText(transcript);
                      }
                    }
                  } catch (e) {
              ...
              handleAnthropicText(msg.text);
            }
          } catch (e) {
            console.error("Error processing client message for Anthropic:", e);
          }
        });

        clientWs.on("close", () => {
          console.log("Client disconnected from Anthropic session");
          if (silenceTimer) clearTimeout(silenceTimer);
        });
      ...
      const session = await ai.live.connect({
        ...
        callbacks: {
          ...
          onerror: (err: any) => {
            ...
            clientWs.send(JSON.stringify({
              type: "error",
              error: `Gemini Live error: ${err.message || String(err)}`
            }));
          },
          onclose: () => {
