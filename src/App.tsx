...
export default function App() {
  ...
  const getThemeButtonColor = () => {
    switch (themeColor) {
      case "violet": return "bg-purple-600 hover:bg-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.4)]";
      case "crimson": return "bg-rose-600 hover:bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.4)]";
      case "emerald": return "bg-emerald-600 hover:bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.4)]";
      case "celestial": return "bg-sky-600 hover:bg-sky-500 shadow-[0_0_10px_rgba(14,165,233,0.4)]";
      case "gold": return "bg-amber-600 hover:bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.4)]";
      case "rose": return "bg-pink-600 hover:bg-pink-500 shadow-[0_0_10px_rgba(244,63,94,0.4)]";
      case "charcoal":
      default:
        return "bg-indigo-600 hover:bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.4)]";
    }
  };

  const getThemeFocusBorderColor = () => {
    switch (themeColor) {
      case "violet": return "focus-within:border-purple-500/40 focus-within:shadow-[0_0_15px_rgba(168,85,247,0.15)]";
      case "crimson": return "focus-within:border-rose-500/40 focus-within:shadow-[0_0_15px_rgba(244,63,94,0.15)]";
      case "emerald": return "focus-within:border-emerald-500/40 focus-within:shadow-[0_0_15px_rgba(16,185,129,0.15)]";
      case "celestial": return "focus-within:border-sky-500/40 focus-within:shadow-[0_0_15px_rgba(14,165,233,0.15)]";
      case "gold": return "focus-within:border-amber-500/40 focus-within:shadow-[0_0_15px_rgba(245,158,11,0.15)]";
      case "rose": return "focus-within:border-pink-500/40 focus-within:shadow-[0_0_15px_rgba(244,63,94,0.15)]";
      case "charcoal":
      default:
        return "focus-within:border-indigo-500/40 focus-within:shadow-[0_0_15px_rgba(99,102,241,0.15)]";
    }
  };

  const handleSendText = (e: FormEvent) => {
    e.preventDefault();
    if (!textInput.trim() || !sessionRef.current || state === "disconnected") return;
    const typedText = textInput.trim();
    setUserCaption(typedText);
    setModelCaption("");
    setCharacterState("thinking");
    sessionRef.current.sendTextMessage(typedText);
    setTextInput("");
  };
  ...
  return (
    ...
    >
      ...
      <footer className="relative z-10 w-full max-w-2xl mx-auto flex flex-col items-center gap-5 mt-auto">
        ...
        {/* Keyboard Text Chat Input Box */}
        {state !== "disconnected" && (
          <form 
            onSubmit={handleSendText}
            className={`w-full max-w-md flex items-center gap-2 p-1.5 rounded-full border border-white/10 bg-white/5 backdrop-blur-md shadow-[0_0_15px_rgba(255,255,255,0.02)] transition-all duration-300 ${getThemeFocusBorderColor()}`}
          >
            <input
              type="text"
              value={textInput}
              ...
              className="flex-1 bg-transparent border-none outline-none pl-4 pr-2 py-1 text-sm text-white placeholder-white/30"
            />
            <button
              type="submit"
              disabled={!textInput.trim()}
              className={`p-2 rounded-full transition-all duration-300 cursor-pointer text-white flex items-center justify-center ${
                textInput.trim() 
                  ? getThemeButtonColor()
