...
type SettingsTab = "general" | "voice" | "api-key" | "system" | "about";
...
export function SettingsPanel({ isOpen, onClose, settings, onChange, themeColor }: SettingsPanelProps) {
  ...
  const [openaiApiKeySuccess, setOpenaiApiKeySuccess] = useState(false);
  ...
  const [tempAnthropicApiKey, setTempAnthropicApiKey] = useState("");
  const [anthropicApiKeySaving, setAnthropicApiKeySaving] = useState(false);
  const [anthropicApiKeySuccess, setAnthropicApiKeySuccess] = useState(false);
  const [anthropicApiKeyError, setAnthropicApiKeyError] = useState<string | null>(null);
  ...
  const handleSaveOpenAiApiKey = async () => {
    const key = tempOpenAiApiKey.trim();
    if (!key || openaiApiKeySaving) return;
    setOpenaiApiKeySaving(true);
    setOpenaiApiKeyError(null);
    setOpenaiApiKeySuccess(false);
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Could not save OpenAI key.");
      setOpenaiApiKeySuccess(true);
      setHasOpenAiApiKey(true);
      setTempOpenAiApiKey("");
      setTimeout(() => setOpenaiApiKeySuccess(false), 3000);
    } catch (err: any) {
      setOpenaiApiKeyError(err.message || "Failed to save OpenAI API key.");
    } finally {
  ...
  const handleSaveAnthropicApiKey = async () => {
    const key = tempAnthropicApiKey.trim();
    if (!key || anthropicApiKeySaving) return;
    setAnthropicApiKeySaving(true);
    setAnthropicApiKeyError(null);
    setAnthropicApiKeySuccess(false);

    try {
      const res = await fetch("/api/config/anthropic", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ apiKey: key }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Could not save Anthropic key.");
      setAnthropicApiKeySuccess(true);
      setHasAnthropicApiKey(true);
      setTempAnthropicApiKey("");
      setTimeout(() => setAnthropicApiKeySuccess(false), 3000);
    } catch (err: any) {
      setAnthropicApiKeyError(err.message || "Failed to save Anthropic API key.");
    } finally {
                  ...
                  <div className="space-y-1.5 text-left border-b border-white/5 pb-4">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 px-3 py-2 rounded-xl border border-white/10 bg-white/5 text-sm text-white font-mono focus:outline-none focus:border-cyan-400/50 transition"
                      />
                      <button
                        onClick={handleSaveApiKey}
                        disabled={apiKeySaving || !tempApiKey.trim()}
                        className="px-4 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black text-xs font-mono font-bold tracking-wider transition disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shrink-0"
                      >
                        {apiKeySaving ? "SAVING..." : "SAVE"}
                      </button>
                    </div>
                    <span className="text-[8px] text-slate-500 uppercase font-mono">
                      Stored locally in secrets.json. Used for Google Gemini Live model.
                    </span>
                    {apiKeySuccess && (
                      <div className="mt-2 p-2 rounded-lg border border-emerald-500/20 bg-emerald-500/5 flex items-center gap-1.5">
                        <Check size={12} className="text-emerald-400 shrink-0" />
                        <span className="text-[9px] font-mono text-emerald-300/80">
                          Gemini key validated and updated successfully.
                        </span>
                      </div>
                    )}
                    {apiKeyError && (
                      <div className="mt-2 p-2 rounded-lg border border-rose-500/20 bg-rose-500/5 flex items-center gap-1.5">
                        <AlertTriangle size={12} className="text-rose-400 shrink-0" />
                        <span className="text-[9px] font-mono text-rose-300/80 leading-relaxed">
                          {apiKeyError}
                        </span>
                      </div>
                    )}
                  ...
                  <div className="space-y-1.5 text-left border-b border-white/5 pb-4">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder={hasOpenAiApiKey ? "••••••••••••••••" : "sk-..."}
                        className="flex-1 px-3 py-2 rounded-xl border border-white/10 bg-white/5 text-sm text-white font-mono focus:outline-none focus:border-cyan-400/50 transition"
                      />
                      <button
                        onClick={handleSaveOpenAiApiKey}
                        disabled={openaiApiKeySaving || !tempOpenAiApiKey.trim()}
                        className="px-4 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black text-xs font-mono font-bold tracking-wider transition disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shrink-0"
                      >
                        {openaiApiKeySaving ? "SAVING..." : "SAVE"}
                      </button>
                    </div>
                    <span className="text-[8px] text-slate-500 uppercase font-mono">
                      Stored locally in secrets.json. Used for GPT-4o Realtime API and TTS.
                    </span>
                    {openaiApiKeySuccess && (
                      <div className="mt-2 p-2 rounded-lg border border-emerald-500/20 bg-emerald-500/5 flex items-center gap-1.5">
                        <Check size={12} className="text-emerald-400 shrink-0" />
                        <span className="text-[9px] font-mono text-emerald-300/80">
                          OpenAI key validated and updated successfully.
                        </span>
                      </div>
                    )}
                    {openaiApiKeyError && (
                  ...
                  <div className="space-y-1.5 text-left pb-4">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder={hasAnthropicApiKey ? "••••••••••••••••" : "sk-ant-..."}
                        className="flex-1 px-3 py-2 rounded-xl border border-white/10 bg-white/5 text-sm text-white font-mono focus:outline-none focus:border-cyan-400/50 transition"
                      />
                      <button
                        onClick={handleSaveAnthropicApiKey}
                        disabled={anthropicApiKeySaving || !tempAnthropicApiKey.trim()}
                        className="px-4 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black text-xs font-mono font-bold tracking-wider transition disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shrink-0"
                      >
                    ...
                    <span className="text-[8px] text-slate-500 uppercase font-mono">
                      Stored locally in secrets.json. Used for Claude 3.5 Sonnet simulated call.
                    </span>
                    {anthropicApiKeySuccess && (
                      <div className="mt-2 p-2 rounded-lg border border-emerald-500/20 bg-emerald-500/5 flex items-center gap-1.5">
                        <Check size={12} className="text-emerald-400 shrink-0" />
                        <span className="text-[9px] font-mono text-emerald-300/80">
                          Anthropic key validated and updated successfully.
                        </span>
                      </div>
                    )}
                    {anthropicApiKeyError && (
                      <div className="mt-2 p-2 rounded-lg border border-rose-500/20 bg-rose-500/5 flex items-center gap-1.5">
                        <AlertTriangle size={12} className="text-rose-400 shrink-0" />
                        <span className="text-[9px] font-mono text-rose-300/80 leading-relaxed">
                          {anthropicApiKeyError}
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5 text-left border-b border-white/5 pb-4">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        onChange({ aiProvider: e.target.value as any });
                        void fetch("/api/settings", {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({ aiProvider: e.target.value }),
                        }).catch(() => {});
                      }}
                    ...
                    >
                      <option value="gemini">Google Gemini Live (Voice &amp; Vision)</option>
                      <option value="openai">OpenAI GPT-4o Realtime (Voice)</option>
                      <option value="anthropic">Anthropic Claude 3.5 Sonnet (Simulated Call)</option>
                    </select>
                    <span className="text-[8px] text-slate-500 uppercase font-mono">
                      Select which AI provider drives MYRAA's intelligence
                    </span>
                  </div>
