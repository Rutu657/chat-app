...
export class MyraaAudioSession {
  ...
  private receivedAudioForThisTurn = false;
    ...
    try {
      ...
      this.ws.onopen = async () => {
        ...
        try {
          ...
          // Obtain User Microphone layout (Optional for keyboard/text-only conversation)
          try {
            ...
            console.warn("Microphone not available or access denied. Operating in text-input mode.", micErr);
          }
      ...
      this.ws.onmessage = async (event) => {
        try {
          ...
          if (data.type === "status") {
            ...
            if (data.status === "connecting_gemini" || data.status === "connecting_openai") {
              // Wait for connection
          ...
          if (data.type === "audio" && data.audio) {
            this.receivedAudioForThisTurn = true;
          ...
          if (data.type === "transcription") {
            ...
            if (data.role === "user") {
              this.receivedAudioForThisTurn = false;
            } else if (data.role === "model") {
              const settings = loadSettings();
              if (settings.aiProvider === "anthropic" && !this.receivedAudioForThisTurn) {
                this.speakTextLocal(data.text);
              }
            }
          }
  ...
  /**
   * Sends keyboard text input to the server/Gemini Live session.
   */
  ...
  private speakTextLocal(text: string) {
    ...
    utterance.rate = 0.95; // slightly slower for cozy heroine pace
    
    // Select a nice female/anime-like voice if possible
